...
class SimpleDocumentProcessor:
    """Simplified document processor that works without heavy ML dependencies"""
    ...
    def _load_embedding_model(self):
        """Load a simple embedding model or use fallback"""
        try:
    ...
    def create_simple_embeddings(self, texts: List[str]) -> List[List[float]]:
        """Create simple embeddings using word frequency or fallback to random"""
        embeddings = []
        ...
        for text in texts:
            if self.embedding_model:
                try:
                    ...
                    logger.warning(f"Failed to create embedding with model: {e}")
                    # Fallback to simple word frequency
                    embeddings.append(self._create_word_frequency_embedding(text))
            else:
                # Use simple word frequency as embedding
                embeddings.append(self._create_word_frequency_embedding(text))
        
        return embeddings
    
    def _create_word_frequency_embedding(self, text: str, dimension: int = 384) -> List[float]:
    ...
    def process_document(self, db: Session, document: Document) -> bool:
        ...
        try:
            ...
            embeddings = self.create_simple_embeddings(sections)
